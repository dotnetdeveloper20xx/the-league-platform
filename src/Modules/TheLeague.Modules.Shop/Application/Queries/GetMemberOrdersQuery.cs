...
public record GetMemberOrdersQuery(
    Guid MemberId,
    int Page = 1,
    int PageSize = 20
) : IRequest<PagedResult<OrderDto>>;

public class GetMemberOrdersQueryHandler : IRequestHandler<GetMemberOrdersQuery, PagedResult<OrderDto>>
{
    private readonly ShopDbContext _db;

    public GetMemberOrdersQueryHandler(ShopDbContext db)
    {
        _db = db;
    }

    public async Task<PagedResult<OrderDto>> Handle(GetMemberOrdersQuery request, CancellationToken cancellationToken)
    {
